      ...
      <div class="visuals">
        <figure class="board-panel">
          <svg
            ...
            viewBox="0 0 500 500"
          >
            <circle class="board-shadow" cx={BOARD_CENTER} cy={BOARD_CENTER} r={BLACK_BORDER_RADIUS + 12} />
            <circle class="board-rim" cx={BOARD_CENTER} cy={BOARD_CENTER} r={BLACK_BORDER_RADIUS} />

            {#each SECTOR_ORDER as sector, index}
              {@const centerAngle = index * 18}
              {@const startAngle = centerAngle - 9}
              {@const endAngle = centerAngle + 9}
              {@const ringClass = index % 2 === 0 ? 'ring-red' : 'ring-green'}
              {@const singleClass = index % 2 === 0 ? 'single-light' : 'single-dark'}
              {@const dividerEnd = polarPoint(BLACK_BORDER_RADIUS, centerAngle + 9)}
              {@const labelPoint = polarPoint(195, centerAngle)}

              <path class={singleClass} d={ringSlicePath(SINGLE_BULL_RADIUS, DOUBLE_INNER_RADIUS, startAngle, endAngle)} />
              <path class={ringClass} d={ringSlicePath(TRIPLE_INNER_RADIUS, TRIPLE_OUTER_RADIUS, startAngle, endAngle)} />
              <path class={ringClass} d={ringSlicePath(DOUBLE_INNER_RADIUS, DOUBLE_OUTER_RADIUS, startAngle, endAngle)} />
              <line class="sector-divider" x1={BOARD_CENTER} y1={BOARD_CENTER} x2={dividerEnd.x} y2={dividerEnd.y} />
              <text class="sector-label" x={labelPoint.x} y={labelPoint.y}>{sector}</text>
            {/each}

            <circle class="bull-outer" cx={BOARD_CENTER} cy={BOARD_CENTER} r={SINGLE_BULL_RADIUS} />
            <circle class="bull-inner" cx={BOARD_CENTER} cy={BOARD_CENTER} r={DOUBLE_BULL_RADIUS} />

            {#if payload}
            <g>
              <circle class="impact-ring" cx={displayImpactX} cy={displayImpactY} r="8" />
              <circle class="impact-core" cx={displayImpactX} cy={displayImpactY} r="4" />
            </g>
            {/if}
          </svg>
        </figure>
